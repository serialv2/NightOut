...
{
    private const string DefaultChannelId = "spotiz_default_channel";
    private const string DefaultChannelName = "Spotiz";
    ...
    public override void OnMessageReceived(RemoteMessage message)
    {
        ...
        try
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("[FCM] Push recu au premier plan : notification Android ignoree, badge interne utilise");
                return;
            }

            // Android affiche deja les payloads FCM "notification" en arriere-plan.
            // Si on cree aussi une notification locale ici, l'utilisateur la recoit en double.
            if (message.GetNotification() is not null)
            {
                System.Diagnostics.Debug.WriteLine("[FCM] Notification systeme deja geree par Android : notification locale ignoree");
                return;
            }
            ...
            title = string.IsNullOrWhiteSpace(title) ? "Spotiz" : title;
