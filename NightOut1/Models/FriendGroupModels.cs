...
public class FriendGroupOuting : BaseModel
{
    ...
    [JsonIgnore] public string? DisplayPlaceName { get; set; }
    [JsonIgnore] public string? DisplayStatsLabel { get; set; }

    // Permet de mélanger les anciennes sorties de groupe et les nouvelles sorties éphémères visibility='group'
    [JsonIgnore] public string SourceType { get; set; } = "group_outing";
    [JsonIgnore] public string SourceId { get; set; } = string.Empty;
    [JsonIgnore] public bool IsEphemeralEvent => string.Equals(SourceType, "ephemeral", StringComparison.OrdinalIgnoreCase);

    [JsonIgnore] public string BarName => Bar?.Name ?? DisplayPlaceName ?? "Lieu à confirmer";
    ...
    [JsonIgnore] public string StatsLabel => DisplayStatsLabel ?? $"✅ {YesCount}   🤔 {MaybeCount}   ❌ {NoCount}";
}
