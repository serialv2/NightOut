...
public sealed class BeaconAutoCheckinService : IDisposable
{
    ...
    private const string SpotizBeaconUuid = "8F2A6B7C-9D31-4E42-AF8B-4D7C2E9F5A10";
    ...
    private async Task ScanOnceAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var matchedSpotizBeacon = false;

            foreach (var advertisement in advertisements)
            {
                if (!string.Equals(
                        BeaconKey.NormalizeUuid(advertisement.Uuid),
                        SpotizBeaconUuid,
                ...
                matchedSpotizBeacon = true;

                if (advertisement.Rssi < DefaultMinRssi)
                {
                    await ShowDiagnosticToastAsync(
                        $"Tag Spotiz trop faible ({advertisement.Rssi} dBm).",
                        now);
                    continue;
                }
                ...
                await ShowDiagnosticToastAsync(
                    $"Tag Spotiz detecte ({advertisement.Rssi} dBm).",
                    now);
            }

            if (advertisements.Count > 0 && !matchedSpotizBeacon)
            {
                var first = advertisements.OrderByDescending(a => a.Rssi).First();
                await ShowDiagnosticToastAsync(
                    $"Beacon vu non Spotiz: {first.Uuid} / {first.Major} / {first.Minor}",
                    now);
            }
