...
public class RealtimeService(SupabaseClient supabase) : IRealtimeService
{
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    var result = await supabase.From<Checkin>()
                        .Where(c => c.BarId == barId && c.IsActive).Get();
    ...
    public async Task SubscribeToFriendMapLocationsAsync(Action onFriendMapUpdated)
    {
        var channel = supabase.Realtime.Channel("profiles:friend-map-locations");
        ...
        channel.AddPostgresChangeHandler(ListenType.All, (_, _) =>
            MainThread.BeginInvokeOnMainThread(onFriendMapUpdated));
