...
namespace NightOut.Services
{
    public class BarService : IBarService
    {
        ...
        public async Task<Dictionary<string, int>> GetActiveCheckinCountsByBarAsync(IEnumerable<string> barIds)
        {
            var ids = (barIds ?? Array.Empty<string>())
                .Where(id => !string.IsNullOrWhiteSpace(id))
                .Distinct()
                .ToHashSet();
            ...
            try
            {
                ...
                return (result?.Models ?? new List<Checkin>())
                    .Where(c => ids.Contains(c.BarId))
                    .GroupBy(c => c.BarId)
                    .ToDictionary(g => g.Key, g => g.Count());
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[BarService] GetActiveCheckinCountsByBar erreur : {ex}");
                return new Dictionary<string, int>();
            }
        }
