...
public class DirectMessageService(SupabaseClient supabase, IAuthService auth, INotificationService notifications) : IDirectMessageService
{
    ...
    public async Task MarkAllConversationsReadAsync()
    {
        try
        {
            var conversations = await GetConversationsAsync();
            var unreadPartnerIds = conversations
            ...
            foreach (var partnerId in unreadPartnerIds)
            {
                try
                {
                    await supabase.Rpc("mark_conversation_read", new { p_partner_id = partnerId });
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[DM] MarkAllConversationsRead item erreur : {ex.Message}");
                }
            }

            await notifications.MarkReadByTypeAsync("private_message", "direct_message");
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[DM] MarkAllConversationsRead erreur : {ex.Message}");
        }
    }
