...
public class AuthService(Client supabase) : IAuthService
{
        ...
        try
        {
            ...
            if (session?.User == null)
            {
                return AuthOperationResult.Fail(
                    "empty_session",
                    "Connexion impossible. Vérifie tes identifiants puis réessaie.");
            }
            ...
            if (_currentProfile == null)
            {
                ...
                if (repaired == null)
                {
                    ...
                    return AuthOperationResult.Fail(
                        "profile_missing",
                        "Ton compte existe, mais ton profil Spotiz est incomplet. Réessaie dans quelques secondes ou contacte le support.");
                }
        ...
        try
        {
            ...
            if (session?.User == null)
            {
                return AuthOperationResult.Fail(
                    ...
                    "Le compte n'a pas pu être créé. Réessaie dans quelques instants.");
            }
            ...
            if (_currentProfile == null)
            {
                return AuthOperationResult.Fail(
                    ...
                    "Ton compte a été créé, mais le profil Spotiz n'a pas pu être finalisé. Ferme puis relance l'application.");
            }
    ...
    public async Task<AuthOperationResult> SignInWithGoogleResultAsync(string accountType = "user")
    {
        try
        {
                ...
                !TryGet(result, "refresh_token", out var refreshToken))
            {
                return AuthOperationResult.Fail(
                    ...
                    "Connexion Google impossible. Aucun jeton de connexion reçu.");
            }
            ...
            if (session?.User == null)
            {
                return AuthOperationResult.Fail(
                    ...
                    "Connexion Google impossible. Réessaie dans quelques instants.");
            }
            ...
            if (_currentProfile == null)
            {
                return AuthOperationResult.Fail(
                    ...
                    "Connexion Google réussie, mais le profil Spotiz n'a pas pu être finalisé.");
            }
        ...
        {
            return AuthOperationResult.Cancelled("Connexion Google annulée.");
        }
        catch (Exception ex)
        {
