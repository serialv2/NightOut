...
{
    public async Task<List<OfficialEvent>> GetMyOfficialEventsAsync()
    {
    ...
    public async Task<List<ProEventDemographicStats>> GetMyEventDemographicStatsAsync()
    {
        var account = await professionalService.GetCurrentProfessionalAccountAsync();
        ...
        try
        {
            var result = await supabase.From<ProEventDemographicStats>()
                .Filter("professional_account_id", Operator.Equals, account.Id)
        ...
        string? flyerUrl)
    {
        ...
        var bar = await GetLinkedBarAsync(account.Id);
        ...
        var officialEvent = new OfficialEvent
        {
            ...
            BarId = bar.Id,
            CityId = account.CityId ?? bar.CityId,
            Title = title.Trim(),
            ...
            Latitude = account.Latitude ?? bar.Latitude,
            Longitude = account.Longitude ?? bar.Longitude,
            Status = "published",
            IsActive = true,
    ...
    public async Task<string?> UploadFlyerAsync(string professionalAccountId, FileResult file)
    {
        ...
        var path = $"official_events/{professionalAccountId}/flyer_{Guid.NewGuid():N}.jpg";
