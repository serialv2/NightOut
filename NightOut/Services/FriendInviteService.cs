...
public class FriendInviteService(Client supabase, IAuthService auth, IFriendService friends, ICreditService credits, INotificationService notifications) : IFriendInviteService
{
    public async Task<FriendInvite?> CreateInviteAsync()
    {
        ...
        try
        {
            ...
            await credits.AddMyCreditsAsync(50, "friend_invite_created", created.Id, "friend_invite");
            return created;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> UseInviteAsync(string inviteCode)
    {
        ...
        try
        {
            ...
            await credits.AddCreditsAsync(invite.InviterId, 500, "friend_invite_used", invite.Id, "friend_invite");
            await credits.AddMyCreditsAsync(100, "friend_invite_used", invite.Id, "friend_invite");
