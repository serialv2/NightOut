...
public partial class FriendsViewModel : BaseViewModel
{
    ...
    private async Task LoadSelectedGroupDetailsCoreAsync(bool markAsRead)
    {
        ...
        await Task.WhenAll(membersTask, messagesTask, outingsTask);
        ...
        SelectedGroupOutings.Clear();
        foreach (var outing in outingsTask.Result) SelectedGroupOutings.Add(outing);
    ...
    private async Task RespondToOutingAsync(FriendGroupOuting? outing, string status)
    {
        ...
        var ok = await _groups.RespondToOutingAsync(outing.Id, status);
        if (ok)
        {
            await LoadSelectedGroupDetailsAsync();
            await LoadInviteAndCreditsAsync();
...
public class FriendItem
{
    ...
    public string StatusDot => Status switch { "out" => "🟡", "online" => "🟢", _ => "⚫" };
    public string StatusLabel => Status switch { "out" => "Dans un bar", "online" => "En ligne", _ => "Hors ligne" };
