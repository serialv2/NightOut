...
Namespace Forms
    ...
    Public Class EventActivityForm
        Inherits Form

        Private ReadOnly _event As EventsForm.AdminEventRow
        ...
        Public Sub New(ev As OfficialEvent)
            Me.New(EventsForm.AdminEventRow.FromOfficial(ev))
        End Sub

        Public Sub New(ev As EphemeralEvent)
            Me.New(EventsForm.AdminEventRow.FromEphemeral(ev))
        End Sub

        Public Sub New(ev As EventsForm.AdminEventRow)
            _event = ev

            Me.Text = "Statistiques evenement"
            ...
            lblTitle.Text = "Stats de " & If(_event?.Title, "evenement")
            ...
            lblSubtitle.Text = $"{If(_event?.TypeLabel, "Evenement")} - {If(_event?.PlaceLabel, "Lieu")} - {If(_event?.DateLabel, "date non definie")} - ID {_event?.Id}"

            lblStatus.AutoSize = True
            lblStatus.ForeColor = NightOutTheme.Cream
            lblStatus.Font = NightOutTheme.FontBody(9.5F)
            lblStatus.Location = New Point(20, 76)
            lblStatus.Text = "Chargement des statistiques..."
            ...
            AddHandler Me.Shown, AddressOf EventActivityForm_Shown
        End Sub

        Private Async Sub EventActivityForm_Shown(sender As Object, e As EventArgs)
        ...
        Private Async Function LoadActivityAsync() As Task
            Try
                ...
                Dim data = Await EventActivityService.GetAsync(_event)
                For Each section In data.Sections
                    AddSection(section)
                Next

                Dim totalRows = data.Sections.Sum(Function(s) s.Rows.Count)
                lblStatus.Text = $"{data.Sections.Count} section(s) - {totalRows} ligne(s)"
            ...
            Catch ex As Exception
                ...
                MessageBox.Show("Impossible de charger les statistiques : " & ex.Message, "Statistiques evenement", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Finally
                Me.UseWaitCursor = False
            End Try
        End Function

        Private Sub AddSection(section As EventActivitySection)
            ...
            If section.Rows.Count = 0 Then
                grid.Rows.Add(Enumerable.Range(0, section.Columns.Length).Select(Function(i) If(i = 0, "Aucune donnee", "")).ToArray())
            Else
